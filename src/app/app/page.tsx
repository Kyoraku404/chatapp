  ...
  return (
    <div className="chat-shell flex h-dvh flex-col bg-paper text-ink-900">
      ...
      <div className="flex min-h-0 flex-1 flex-col md:hidden">
        {mobileView === "list" ? (
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <header className="flex items-center gap-2 border-b border-ink-100 px-3 py-2.5">
              <button onClick={() => setMobileView("list")} aria-label="Back to conversations" className="grid h-11 w-11 place-items-center rounded-xl hover:bg-white">
        ...
        {mobileView === 'list' && <MobileNav active={rail} unread={demoTotal} onSelect={id => { setRail(id); setMobileView('list'); }} />}
      </div>
    </div>
  );
}
